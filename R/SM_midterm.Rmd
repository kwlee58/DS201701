---
title: "SM Midterm 201701"
author: "coop711"
date: "`r Sys.Date()`"
output: html_document
---

#### Data 

#### Excel 자료 읽어들이기 

* excel 파일을 잘 읽어들이는 `xlsx` 패키지를 올려놓고, 자료구조에 맞춰 argument 설정.
    * Sheet 번호, startRow, colIndex, header 의 용도 파악. 

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
library(xlsx)
library(pander)
library(knitr)
panderOptions('table.split.table', Inf)
panderOptions('table.alignment.rownames', 'left')
panderOptions('table.alignment.default', 'right')
options(width=180)
# panderOptions('digits', 3)
```

<!--
권현종 학생의 위치가 100번쨰에 놓여 있어서 결석생들의 위치만 가지고 group을 나눠줄 수 없었음. 그들의 id를 다시 찾아서  id와 일치하는 group을 부여하게 됨.
-->

```{r, read data, echo = TRUE, results = 'markup'}
literacy.roll <- read.xlsx("../data/SM_midterm.xlsx", 1, startRow = 3, endRow  = 32, colIndex = c(3:7, 9:42), colClasses = c(rep("character", 39)), header = FALSE, stringsAsFactors = FALSE)
names(literacy.roll) <- c("dept", "id", "name", "year", "e.mail", "cell.no", paste("Q", 1:33, sep = ""))
head(literacy.roll)
str(literacy.roll)
literacy <- literacy.roll[, c(7:39)]
kable(literacy)
```
### Q4 수정

```{r, Q4}
literacy[literacy == "1"] <- "가"
literacy[literacy == "2"] <- "나"
literacy[literacy == "3"] <- "다"
literacy[literacy == "4"] <- "라"
literacy$Q4 <- tolower(literacy$Q4)
na.index <- which(is.na(literacy$Q4))
literacy$Q4[na.index] <- "9999"
q4.split <- sapply(literacy$Q4, function(x) unlist(strsplit(x, split = "")))
Q4_m <- t(q4.split)
rownames(Q4_m) <- NULL
colnames(Q4_m) <- paste("Q4", 1:4, sep = ".")
Q4_m[Q4_m == "9"] <- NA
Q4_m
SM_midterm <- data.frame(literacy[, 1:3], Q4_m, literacy[, -(1:4)], row.names = NULL, stringsAsFactors = FALSE)
pander(SM_midterm)
```

```{r, pander literacy, echo = TRUE}
pander(literacy)
```

#### 문해력 점수 계산

* 정답 맞춰보기
    * 정답 읽어들이기. `what=""`로 설정하여 `char`취급. 

```{r, read ans, echo = FALSE, results = 'hide'}
(ans <- scan("../data/SM_ans.txt", what = ""))
```

<!--* 각 학생들의 답안과 맞춰보기 위하여 정답표 작성.
    * 문항별로 정답과 비교하기 위하여 각 문항을 리스트의 원소로 분리
    * `split()` 의 기능과 한계에 유의. `factor` 성격으로부터 `1, 10, 11, ..., 2, 20, ..., 3, 4, 5, ... , 9`로 순서를 잡게 되므로 다시 한번 정리할 필요.
-->
    
    
```{r, ans list and unlist, echo = FALSE, results = 'markup'}
(ans.l <- split(ans, f = paste("A", c(1:3, paste(4, 1:4, sep = "."), 5:33), sep = "")))
(ans.l.2 <- ans.l[paste("A", c(1:3, paste(4, 1:4, sep = "."), 5:33), sep = "")])
```

##### 정답과 대조하여 R(Right)/W(Wrong) 표시 
    
<!--* 채점표라는 의미의 `literacy.marks` 로 저장, (# 표시한 방식으로는 작동하지 않음)-->
    
```{r, marks, echo = TRUE}
# 첫 세 학생의 응답을 기초로 확대, mapply()의 활용 방식에 유의
literacy.marks.rw.q1_3 <- mapply(function(x, y) factor(ifelse(x == y, "R", "W")), y = ans.l.2[1:3], SM_midterm[, 1:3])
literacy.marks.rw <- mapply(function(x, y) factor(ifelse(x == y, "R", "W")), y = ans.l.2, SM_midterm)
# literacy.marks.rw <- mapply(function(x, y = ans.l.2) factor(ifelse(x == y, "R", "W")), literacy[, -26])
# literacy.marks.rw <- sapply(literacy[, -26], function(x, y = ans.l.2) factor(ifelse(x == y, "R", "W")))
pander(literacy.marks.rw)
```
<P style = "page-break-before:always">

##### 학생별 점수 산출

<!--+ 정답을 집계하기 위하여 "R" 갯수를 다시 산출하는 `ifelse()` 적용.
    + 4점씩 곱하면 100점 만점 기준의 점수가 됨. 
    + 조사표는 작성해서 제출했지만 빈칸이 있는 경우는 그 문항을 0점 처리, 결석의 경우는 점수가 모두 0으로 나오는 경우를  NA처리 
-->

```{r, sum of rights answers, echo = FALSE, results = 'markup'}
literacy.marks <- ifelse(literacy.marks.rw == "R", 1, 0)
pander(literacy.marks)
score <- c(rep(3, 3), rep(1, 4), rep(3, 29))
literacy.score <- literacy.marks %*% score
pander(data.frame(학번 = literacy.roll$id, 점수 = literacy.score))
# literacy.score <- apply(literacy.marks, 1, sum, na.rm = TRUE) * 4
# literacy.score[literacy.score == 0] <- NA
which(literacy.score < 60)
```

```{r, pander score, echo = FALSE}
# pander(literacy.score)
```

<!--
<P style = "page-break-before:always">


<!--##### 문항별 정답률-->

<!--* 결석한 경우와 일부 문항에 답을 내지 않은 경우를 구분하는 것이 핵심.
    + `literacy.marks`를 그대로 이용하면 쉽게 계산되나, `is.na`학습 목적으로 `literacy.mark.rw`에서 출발
    + 모든 문항에 NA로 답한 결석한 학생들의 인덱스를 먼저 찾아내고,
-->

```{r, NA management, echo = FALSE, results = 'hide'}
apply(literacy.marks.rw, 1, function(x) all(is.na(x)))
na.all <- which(apply(literacy.marks.rw, 1, function(x) all(is.na(x))))
```

<!--* 정답률 계산
    + NA를 모두 오답처리한 후, 결석한 학생들만 다시 NA로 정정.
    + TRUE의 갯수를 합하여 평균. 
-->

```{r, computing correct answer rates, echo = TRUE, results = 'markup'}
literacy.marks.2 <- literacy.marks.rw
literacy.marks.2[is.na(literacy.marks.2)] <- "W"
literacy.marks.2[na.all, ] <- NA
literacy.marks.2 <- ifelse(literacy.marks.2 == "R", 1, 0)
rate.by.item <- apply(literacy.marks.2, 2, mean, na.rm = TRUE)*100
str(rate.by.item)
rate.by.item <- as.matrix(rate.by.item, 25)
dimnames(rate.by.item) <- list("문항" =  paste("문", c(1:3, paste(4, 1:4, sep = "."), 5:33), sep = ""), "정답률" = "정답률")
# colnames(rate.by.item) <- "정답률"
```

<!--
```{r, correct answer rates, echo = FALSE}
pander(rate.by.item, digits = 3)
```
-->

<!--* `replace()`를 이용하면, 

```{r}
(literacy.marks.3 <- replace(literacy.marks.rw, is.na(literacy.marks.rw), "W"))
literacy.marks.3[na.all, ] <- NA
literacy.marks.3
```
-->

<!--<P style = "page-break-before:always">-->

#### 어려운 문제?

##### 정답률 80% 이하

```{r, hardest question below 80, echo = FALSE}
pander(rate.by.item[which(rate.by.item <= 80), ])
```

##### 정답률 70% 이하

```{r, hardest question below 70, echo = FALSE}
pander(rate.by.item[which(rate.by.item <= 70), ])
```

##### 정답률 60% 이하

```{r, hardest question below 60, echo = FALSE}
pander(t(rate.by.item[which(rate.by.item <= 60), , drop = FALSE]))
```

##### 정답률 50% 이하

```{r, hardest question below 50, echo = FALSE}
pander(t(rate.by.item[which(rate.by.item <= 50), , drop = FALSE]))
```

##### 정답률 40% 이하

```{r, hardest question below 40, echo = FALSE}
pander(t(rate.by.item[which(rate.by.item <= 40), , drop = FALSE]))
```

##### 정답률 30% 이하

```{r, hardest question below 30, echo = FALSE}
pander(t(rate.by.item[which(rate.by.item <= 30), , drop = FALSE]))
```


<!--## Data 정리

```{r}
save.image("SM_midterm.RData")
```
-->
